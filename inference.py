...
# ── Single global client (proxy tracks this session) ────────────────────────
client = OpenAI(
...
def _llm_select_action(obs: Observation) -> Optional[Action]:
    ...
    system_prompt = "You are a data cleaning agent. Output ONLY valid JSON with keys: action_type, target_column (optional), parameters (optional dict)."
    user_msg = (
        f"Task: {obs.task_id}\n"
        ...
        "Choose the best action. Output ONLY valid JSON."
    )
